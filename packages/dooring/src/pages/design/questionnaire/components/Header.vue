 ...
 * @LastEditTime: 2024-10-30 14:57:28
-->
<template>
  <div>
    <div class="design-header">
      <div class="flex items-center h-full">
        <el-tooltip content="返回我的问卷" placement="bottom" effect="light">
          <iconpark-icon
      ...
      <div class="flex items-center">
        <!-- github -->
        <a
          class="design-header-item"
          href="https://github.com/wangyuan389/yunda"
          target="_blank"
        >
    ...
    >
      ...
      >
        <input v-model="questionnaire.title" placeholder="请输入问卷标题" />
      </el-form-item>
...
async function createQr() {
  ...
  questionnaire.value.qr = res.data.url;
}

function getQuestionnaireData() {
  http
    .post("/questionnaireData/getList", {
      questionnaireId: questionnaire.value.id,
    })
    .then((res) => {
      formDataLen.value = res.data.list.length;
    });
}
...
async function handlesSubmit() {
  ...
  delete questionnaire.value._id;

  if (!questionnaire.value.qr) {
    console.log("生成二维码");
    await createQr();
  }

  let res = await http.post("/questionnaire/publish", {
    ...questionnaire.value,
