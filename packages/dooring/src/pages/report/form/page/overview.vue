...
const questionTypes = ref([
  ...
  {
    name: "图片上传",
    componentName: "FormUploadWidget",
    value: 0,
  },
]);
...
async function getDetailById() {
  await http.post("/form/getDetailById", { id }).then((res) => {
    ...
    overview.value.topicNum = form.value.formDesign.widgetList.length;

    form.value.formDesign.widgetList.map((item) => {
...
async function getList() {
  await http.post("/formData/getList", { formId: id }).then((res) => {
