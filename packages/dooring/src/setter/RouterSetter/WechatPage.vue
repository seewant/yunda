 ...
 * @LastEditTime: 2024-07-26 11:10:26
-->
<template>
  <div class="page">
    <div class="h-70 lh-70">选择跳转的搭建页面</div>
...
const pageTree = computed(() => {
  const navigationPage = wechat.value.configure.navigation.list
    ...
    .map((item) => item.router.id);
  console.log("navigationPage", navigationPage);
  return [
    {
      name: "搭建页面",
      list: wechat.value.pageList
        .filter((item) => !navigationPage.includes(item.id))
        .map((item) => {
          return {
            name: item.name,
            id: item.id,
